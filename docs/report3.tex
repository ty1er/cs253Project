...
\begin{table}[hb]
  ...
  \begin{tabular}{|c|c|c|}
    ...
    \hline
    required-writes (W) & 3 & The least number of writes without throwing an exception. \\
    \hline
    required-reads (R) & 3 & The least number of reads without throwing an exception. \\
    \hline
  \end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item As soon as all writes in one node is done, the client check the data in its node independently.
  \item Wait for several seconds, recheck the data in each node.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item All nodes go back the result 1 as the network partition is removed.
  \item ...
  \item DUring thier period, each node is exchanging thier data for replicas. Then all nodes have a whole replications of all data, which meets the eventual consistency.
\end{enumerate}
